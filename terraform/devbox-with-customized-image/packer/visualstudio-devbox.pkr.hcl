# Packer configuration for building Visual Studio + VS Code customized Windows image for DevCenter
...
variable "vm_size" {
  type        = string
  description = "Size of the build VM (use larger size for Visual Studio)"
...
source "azure-arm" "visualstudio_devbox" {
...
# Build configuration
build {
  name = "visualstudio-devbox-image"
  sources = [
    "source.azure-arm.visualstudio_devbox"
  ]
  ...
  # Install VS Code and additional development tools via Chocolatey
  provisioner "powershell" {
    inline = [
  ...
  provisioner "powershell" {
    inline = [
      ...
      "Add-MpPreference -ExclusionProcess 'devenv.exe' -ErrorAction SilentlyContinue"
    ]
  }
