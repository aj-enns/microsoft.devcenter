...
build {
  ...
  provisioner "powershell" {
    inline = [
      ...
      "    Write-Host '  ✓ WSL 2 set as default'",
      "} catch {",
      "    Write-Host '  Warning: Could not set WSL 2 as default' -ForegroundColor Yellow",
      "}",
      "",
      "# Update WSL kernel",
      ...
      "} catch {",
      "    Write-Host '  Warning: WSL update may need to run on first boot' -ForegroundColor Yellow",
      "}",
      "",
      "# Try to install Ubuntu - if it fails, that's OK, user can install on first boot",
      "Write-Host 'Installing Ubuntu distribution...'",
      "Write-Host 'Note: This may take several minutes or may require first-boot configuration...'",
      "try {",
      "    # Use --web-download to avoid Microsoft Store issues",
      "    $result = wsl --install -d Ubuntu --web-download 2>&1",
      "    if ($LASTEXITCODE -eq 0) {",
      "        Write-Host '  ✓ Ubuntu installed successfully'",
      "    } else {",
      "        Write-Host '  Ubuntu installation will complete on first boot' -ForegroundColor Yellow",
      "    }",
      "} catch {",
      "    Write-Host '  Ubuntu installation will complete on first boot' -ForegroundColor Yellow",
      "}",
