...
build {
  ...
  provisioner "powershell" {
    inline = [
      ...
      "} else {",
      "    Write-Host ('WSL update returned exit code: ' + $LASTEXITCODE) -ForegroundColor Yellow",
      "}",
      "",
      ...
      "} else {",
      "    Write-Host ('WSL set-default returned exit code: ' + $LASTEXITCODE) -ForegroundColor Yellow",
      "}",
      "",
      ...
      "} elseif ($ubuntuExitCode -eq 1) {",
      "    # Exit code 1 often means already installed or minor issue",
      "    Write-Host ('Ubuntu installation completed with exit code 1 (may already exist)')",
      "} else {",
      "    Write-Host ('Ubuntu installation exit code: ' + $ubuntuExitCode) -ForegroundColor Yellow",
      "}",
      "",
